...
本项目为一个集成了自然语言处理（NLP）与交互式数据可视化的综合型信息系统。系统核心基于 **LSTM (长短期记忆神经网络)** 进行情感倾向分类（正向/负向），并结合规则匹配实现多维度的业务特征提取。系统包含完整的模型训练管线与具备权限管理的 Web 终端，支持单条文本的实时推断以及大规模数据的批量挖掘。系统UI采用Google Material Design风格，简洁、专业、现代。
...
2. **全局监控大屏**：基于 Plotly 驱动的交互式可视化面板，直观呈现总体情感极性占比、业务维度关注度排行及情感交叉分析矩阵。
3. **单文本诊断**：针对单条评论的实时分析引擎。即刻输出情感极性判定、模型置信度以及所涉及的具体业务维度（如“服务体验”、“卫生与设施”等）。
4. **数据批量挖掘**：支持大规模 CSV 格式文本数据的自动化清洗、全量推断、打标及结果包的结构化导出。
5. **高级设置**：在侧边栏提供自定义停用词库的动态扩展功能，用于过滤无意义词汇，提高分析准确性。
