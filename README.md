...
本项目为一个集成了自然语言处理（NLP）与交互式数据可视化的综合型信息系统。系统核心基于 **LSTM (长短期记忆神经网络)** 进行情感倾向分类（正向/负向），并结合规则匹配实现多维度的业务特征提取。系统包含完整的模型训练管线与具备权限管理的 Web 终端，支持单条文本的实时推断以及大规模数据的批量挖掘。系统UI采用Apple设计系统风格，极简、专业、现代。
